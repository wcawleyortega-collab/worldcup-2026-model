...
- **Squad market value** (Transfermarkt total per team, `data/squad_values.csv`) is now blended into the base Elo at a conservative 30% tilt — `tune_squad.py` shows out-of-sample on the WC2026 group stage that it carries information Elo lacks (RPS 0.167→0.160 at the shipped weight; interior optimum confirms real, complementary signal). See `PORTFOLIO.md`. **xG is deliberately not included**: no public per-team international xG history exists for all 48 sides, and the project will not invent data. Squad *age/minutes* covariates remain a future extension.
