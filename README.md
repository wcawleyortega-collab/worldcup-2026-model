...
1. **Elo ratings** (`wc_model/elo.py`) — eloratings.net formulation run over all 49,000+ internationals since 1872: K = 60/50/40/30/20 by match importance, goal-margin multiplier G = 1 / 1.5 / (11+N)/8, +100 home advantage. The base rating is then tilted 30% toward a **squad-market-value-implied rating** (`wc_model/squad.py`), an out-of-sample-validated covariate (see Limitations / `tune_squad.py`).
...
- **Squad market value** (Transfermarkt total per team) is now blended into the base Elo at a conservative 30% tilt — `tune_squad.py` validates it out-of-sample across **three World Cups** (2018/2022/2026, each with its period-correct snapshot in `data/squad_values*.csv`, Elo frozen pre-tournament): it beats pure Elo in all three (mean RPS 0.1991→0.1931 at the shipped weight), with a pooled interior optimum at ~0.3 Elo weight — the signature of real, complementary signal. See `PORTFOLIO.md`. **xG is deliberately not included**: no public per-team international xG history exists for all 48 sides, and the project will not invent data. Squad *age/minutes* covariates remain a future extension.
